...
def search_command():
    list.delete(0,END)
    for row in backend.search(date_text.get(),earnings_text.get(),excercise_text.get(),study_text.get(),diet_text.get(),python_text.get()):
        list.insert(END,row)
...
e2 = Entry(win,textvariable = excercise_text)
e2.grid(row = 1,column = 1)
diet_text = StringVar()
e3 = Entry(win,textvariable = diet_text)
e3.grid(row = 2,column = 1)
earnings_text = StringVar()
e4 = Entry(win,textvariable = earnings_text)
...
list = Listbox(win,height = 8,width = 33)
